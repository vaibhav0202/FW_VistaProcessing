...
class FW_VistaProcessing_Model_Taxwarecodeupdate extends Mage_Core_Model_Abstract
{
    public function import($addProducts, $updateProducts, $processFullFile, $processUpdateFile, $getNewFiles, $deleteFiles, $sendSucessLog)
    {
    	...
    	$successCount = 0;
    	$errorCount = 0;
    	$logFile = 'Vista_Taxwarecode_Update'.date("YmdHis", time()).'.log';
    	$errorFile =  'Vista_Taxwarecode_Error'.date("YmdHis", time()).'.log';
		...
		$processedSkuFile = '/var/importexport/vista_inventory/Taxwaretaxcode_Processed_'.time();
		$deadLockSkuFile = '/var/importexport/vista_inventory/Taxwaretaxcode_DEADLOCK_'.time();
		...
		foreach($products as &$product){
			...
			try{
				if($sku != ""){
					...
					if($inventoryProduct){
						//Product Update
						$this->updateProduct($inventoryProduct, $product);
				...
				if(strstr($e->getMessage(), "1205 Lock wait timeout exceeded")) {
					...
					$errorCount++;
					if($errorCount > 0){
						$this->sendEmail("Vista Taxwarecode Update Error", "There was an error(s) in the Vista Taxwarecode Update process", Mage::getBaseDir()."/var/log/", $errorFile);
					}
					if($sendSucessLog && $successCount > 0){
						$this->sendEmail("Vista Taxwarecode Update", "Metrics for the Vista Taxwarecode Update process", Mage::getBaseDir()."/var/log/", $logFile);
					}
					return;
				}
			}
		...
		Mage::log("************************ Taxware Taxcode Update Completed ************************\r\n",null,$logFile);
    	...
    	if($errorCount > 0)
    	{
			$this->sendEmail('Taxware Taxcode Update Update Error', 'There was an error(s) in the Taxware Taxcode Update process', Mage::getBaseDir().'/var/log/', $errorFile);
    	}
    	
    	if($sendSucessLog && $successCount > 0)
    	{
			$this->sendEmail('Taxware Taxcode Update', 'Metrics for the Taxware Taxcode Update process', Mage::getBaseDir().'/var/log/', $logFile);
    	}
    	
     	if($inventoryFile == NULL)
    	{
			$this->sendEmail('Taxware Taxcode Update - NO FILES FOUND', 'There were no Taxware Taxcode files found to process', Mage::getBaseDir().'/var/log/', null);
    	}
    }
	...
	{
		$email_from = "Magento/VISTA Taxwarecode Update Processer";
     ...
     */
	private function updateProduct($inventoryProduct, $product)
	{
		$inventoryProduct->setData('taxware_taxcode',$product[3]);
